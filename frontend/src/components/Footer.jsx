export default function Footer() {
  return (
    <footer style={{
      background: 'var(--kernia-obsidian)',
      padding: 'clamp(2.5rem, 6vw, 6rem) 0 clamp(1rem, 2vw, 2rem)',
    }}>
      <div style={{
        maxWidth: '1400px',
        margin: '0 auto',
        padding: '0 clamp(1rem, 4vw, 2rem)',
      }}>
        {/* Main Footer Content */}
        <div 
          className="footer-grid"
          style={{
            display: 'grid',
            gridTemplateColumns: '2fr 1fr 1fr 1fr',
            gap: 'clamp(1.5rem, 4vw, 4rem)',
            marginBottom: 'clamp(1.5rem, 4vw, 4rem)',
          }}
        >
          {/* Brand Section */}
          <div>
            <h3 style={{
              fontFamily: 'var(--font-display)',
              fontSize: 'clamp(1.5rem, 3vw, 2rem)',
              fontWeight: 400,
              color: 'var(--kernia-ivory)',
              marginBottom: '1.5rem',
              letterSpacing: '-0.01em',
            }}>
              Zyra
            </h3>
            <p style={{
              fontFamily: 'var(--font-ui)',
              fontSize: '1rem',
              fontWeight: 300,
              color: 'rgba(252,252,250,0.8)',
              lineHeight: 1.6,
              marginBottom: '2rem',
              maxWidth: '300px',
            }}>
              Where timeless elegance meets uncompromising craftsmanship. Each piece tells a story of luxury, heritage, and artisanal excellence.
            </p>
            
            {/* Social Links */}
            <div style={{
              display: 'flex',
              gap: '1.5rem',
            }}>
              {[
                { 
                  name: 'Instagram', 
                  icon: (
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                    </svg>
                  )
                },
                { 
                  name: 'Facebook', 
                  icon: (
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                  )
                },
                { 
                  name: 'Pinterest', 
                  icon: (
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.174-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.347-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.402.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.357-.629-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24.009 12.017 24c6.624 0 11.99-5.367 11.99-12.013C24.007 5.367 18.641.001.017.001z"/>
                    </svg>
                  )
                }
              ].map((social, index) => (
                <a
                  key={index}
                  href={`#${social.name.toLowerCase()}`}
                  style={{
                    width: '40px',
                    height: '40px',
                    border: '1px solid rgba(252,252,250,0.3)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    textDecoration: 'none',
                    transition: 'all 0.3s ease',
                    color: 'rgba(252,252,250,0.7)',
                  }}
                  onMouseEnter={(e) => {
                    e.target.style.borderColor = 'var(--kernia-gold)';
                    e.target.style.backgroundColor = 'var(--kernia-gold)';
                    e.target.style.color = 'var(--kernia-obsidian)';
                  }}
                  onMouseLeave={(e) => {
                    e.target.style.borderColor = 'rgba(252,252,250,0.3)';
                    e.target.style.backgroundColor = 'transparent';
                    e.target.style.color = 'rgba(252,252,250,0.7)';
                  }}
                  title={social.name}
                >
                  {social.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Collections */}
          <div>
            <h4 style={{
              fontFamily: 'var(--font-ui)',
              fontSize: '0.8rem',
              fontWeight: 500,
              color: 'var(--kernia-ivory)',
              letterSpacing: '0.1em',
              textTransform: 'uppercase',
              marginBottom: '1.5rem',
            }}>
              Collections
            </h4>
            <div style={{
              display: 'flex',
              flexDirection: 'column',
              gap: '0.75rem',
            }}>
              {['Heritage', 'Contemporary', 'Limited Edition', 'Seasonal', 'Bespoke'].map((item, index) => (
                <a
                  key={index}
                  href={`/collections/${item.toLowerCase().replace(' ', '-')}`}
                  style={{
                    fontFamily: 'var(--font-ui)',
                    fontSize: '0.9rem',
                    fontWeight: 300,
                    color: 'rgba(252,252,250,0.8)',
                    textDecoration: 'none',
                    transition: 'color 0.3s ease',
                  }}
                  onMouseEnter={(e) => {
                    e.target.style.color = 'var(--kernia-gold)';
                  }}
                  onMouseLeave={(e) => {
                    e.target.style.color = 'rgba(252,252,250,0.8)';
                  }}
                >
                  {item}
                </a>
              ))}
            </div>
          </div>

          {/* Customer Care */}
          <div>
            <h4 style={{
              fontFamily: 'var(--font-ui)',
              fontSize: '0.8rem',
              fontWeight: 500,
              color: 'var(--kernia-ivory)',
              letterSpacing: '0.1em',
              textTransform: 'uppercase',
              marginBottom: '1.5rem',
            }}>
              Customer Care
            </h4>
            <div style={{
              display: 'flex',
              flexDirection: 'column',
              gap: '0.75rem',
            }}>
              {['Contact Us', 'Size Guide', 'Care Instructions', 'Returns', 'Shipping'].map((item, index) => (
                <a
                  key={index}
                  href={`/support/${item.toLowerCase().replace(' ', '-')}`}
                  style={{
                    fontFamily: 'var(--font-ui)',
                    fontSize: '0.9rem',
                    fontWeight: 300,
                    color: 'rgba(252,252,250,0.8)',
                    textDecoration: 'none',
                    transition: 'color 0.3s ease',
                  }}
                  onMouseEnter={(e) => {
                    e.target.style.color = 'var(--kernia-gold)';
                  }}
                  onMouseLeave={(e) => {
                    e.target.style.color = 'rgba(252,252,250,0.8)';
                  }}
                >
                  {item}
                </a>
              ))}
            </div>
          </div>

          {/* Contact Info */}
          <div>
            <h4 style={{
              fontFamily: 'var(--font-ui)',
              fontSize: '0.8rem',
              fontWeight: 500,
              color: 'var(--kernia-ivory)',
              letterSpacing: '0.1em',
              textTransform: 'uppercase',
              marginBottom: '1.5rem',
            }}>
              Atelier
            </h4>
            <div style={{
              display: 'flex',
              flexDirection: 'column',
              gap: '1rem',
            }}>
              <div>
                <p style={{
                  fontFamily: 'var(--font-ui)',
                  fontSize: '0.9rem',
                  fontWeight: 300,
                  color: 'rgba(252,252,250,0.8)',
                  lineHeight: 1.5,
                  marginBottom: '0.5rem',
                }}>
                  123 Artisan Boulevard<br />
                  Florence, Italy 50125
                </p>
              </div>
              <div>
                <p style={{
                  fontFamily: 'var(--font-ui)',
                  fontSize: '0.9rem',
                  fontWeight: 300,
                  color: 'rgba(252,252,250,0.8)',
                  marginBottom: '0.25rem',
                }}>
                  +39 055 123 4567
                </p>
                <p style={{
                  fontFamily: 'var(--font-ui)',
                  fontSize: '0.9rem',
                  fontWeight: 300,
                  color: 'rgba(252,252,250,0.8)',
                }}>
                  atelier@Zyra.com
                </p>
              </div>
              <div>
                <p style={{
                  fontFamily: 'var(--font-ui)',
                  fontSize: '0.75rem',
                  fontWeight: 300,
                  color: 'rgba(252,252,250,0.6)',
                  letterSpacing: '0.05em',
                  textTransform: 'uppercase',
                }}>
                  Mon - Sat: 10:00 - 19:00
                </p>
              </div>
            </div>
          </div>
        </div>

        {/* Newsletter Section */}
        <div 
          className="newsletter-section"
          style={{
            borderTop: '1px solid rgba(252,252,250,0.1)',
            borderBottom: '1px solid rgba(252,252,250,0.1)',
            padding: 'clamp(1.5rem, 4vw, 3rem) 0',
            marginBottom: 'clamp(1.5rem, 4vw, 3rem)',
          }}
        >
          <div 
            className="newsletter-content"
            style={{
              display: 'flex',
              justifyContent: 'space-between',
              alignItems: 'center',
              gap: '2rem',
            }}
          >
            <div>
              <h4 style={{
                fontFamily: 'var(--font-display)',
                fontSize: 'clamp(1.2rem, 2.5vw, 1.5rem)',
                fontWeight: 400,
                color: 'var(--kernia-ivory)',
                marginBottom: '0.5rem',
              }}>
                Stay Connected
              </h4>
              <p style={{
                fontFamily: 'var(--font-ui)',
                fontSize: '0.9rem',
                fontWeight: 300,
                color: 'rgba(252,252,250,0.8)',
              }}>
                Be the first to discover new collections and exclusive events.
              </p>
            </div>
            <div 
              className="newsletter-form"
              style={{
                display: 'flex',
                gap: '1rem',
                alignItems: 'center',
              }}
            >
              <input
                type="email"
                placeholder="Enter your email"
                style={{
                  padding: '1rem 1.5rem',
                  background: 'transparent',
                  border: '1px solid rgba(252,252,250,0.3)',
                  color: 'var(--kernia-ivory)',
                  fontFamily: 'var(--font-ui)',
                  fontSize: '0.9rem',
                  minWidth: 'clamp(200px, 30vw, 250px)',
                  outline: 'none',
                  transition: 'border-color 0.3s ease',
                }}
                onFocus={(e) => {
                  e.target.style.borderColor = 'var(--kernia-gold)';
                }}
                onBlur={(e) => {
                  e.target.style.borderColor = 'rgba(252,252,250,0.3)';
                }}
              />
              <button
                className="subscribe-btn"
                style={{
                  padding: '1rem 2rem',
                  background: 'var(--kernia-gold)',
                  color: 'var(--kernia-obsidian)',
                  border: 'none',
                  fontFamily: 'var(--font-ui)',
                  fontSize: '0.75rem',
                  fontWeight: 500,
                  letterSpacing: '0.1em',
                  textTransform: 'uppercase',
                  cursor: 'pointer',
                  transition: 'all 0.3s ease',
                  whiteSpace: 'nowrap',
                }}
                onMouseEnter={(e) => {
                  e.target.style.background = 'var(--kernia-ivory)';
                }}
                onMouseLeave={(e) => {
                  e.target.style.background = 'var(--kernia-gold)';
                }}
              >
                Subscribe
              </button>
            </div>
          </div>
        </div>

        {/* Bottom Footer */}
        <div 
          className="footer-bottom"
          style={{
            display: 'flex',
            justifyContent: 'space-between',
            alignItems: 'center',
            flexWrap: 'wrap',
            gap: '1rem',
          }}
        >
          <p style={{
            fontFamily: 'var(--font-ui)',
            fontSize: '0.75rem',
            fontWeight: 300,
            color: 'rgba(252,252,250,0.6)',
          }}>
            © 2024 Zyra. All rights reserved.
          </p>
          <div 
            className="footer-links"
            style={{
              display: 'flex',
              gap: 'clamp(1rem, 3vw, 2rem)',
            }}
          >
            {['Privacy Policy', 'Terms of Service', 'Cookie Policy'].map((item, index) => (
              <a
                key={index}
                href={`/${item.toLowerCase().replace(' ', '-')}`}
                style={{
                  fontFamily: 'var(--font-ui)',
                  fontSize: '0.75rem',
                  fontWeight: 300,
                  color: 'rgba(252,252,250,0.6)',
                  textDecoration: 'none',
                  transition: 'color 0.3s ease',
                }}
                onMouseEnter={(e) => {
                  e.target.style.color = 'var(--kernia-gold)';
                }}
                onMouseLeave={(e) => {
                  e.target.style.color = 'rgba(252,252,250,0.6)';
                }}
              >
                {item}
              </a>
            ))}
          </div>
        </div>
      </div>
      
      {/* Responsive Styles */}
      <style>{`
        @media (max-width: 1024px) {
          .footer-grid {
            grid-template-columns: 1fr 1fr !important;
            gap: 3rem !important;
          }
        }
        
        @media (max-width: 768px) {
          .footer-grid {
            grid-template-columns: 1fr !important;
            gap: 2rem !important;
            text-align: center !important;
          }
          
          .newsletter-content {
            flex-direction: column !important;
            text-align: center !important;
          }
          
          .newsletter-form {
            flex-direction: column !important;
            width: 100% !important;
          }
          
          .newsletter-form input {
            width: 100% !important;
            min-width: auto !important;
          }
          
          .subscribe-btn {
            width: 100% !important;
          }
          
          .footer-bottom {
            flex-direction: column !important;
            text-align: center !important;
          }
          
          .footer-links {
            flex-wrap: wrap !important;
            justify-content: center !important;
          }
        }
      `}</style>
    </footer>
  );
}
