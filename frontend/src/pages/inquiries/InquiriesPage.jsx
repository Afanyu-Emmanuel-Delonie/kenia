...
function ReplyButtons({ inquiry }) {
  const productLabel = inquiry.productTitle || "General enquiry";
  ...
  const openWhatsApp = () => {
    ...
    const text = encodeURIComponent(
      `${greeting}Regarding your inquiry:\n"${inquiry.message}"\n\nZyra Atelier`,
    );
  ...
  const openEmail = () => {
    const subject = encodeURIComponent(
      `Re: Your inquiry about ${productLabel}`,
    );
    const body = encodeURIComponent(
      `${greeting}Regarding your inquiry:\n"${inquiry.message}"\n\nZyra Atelier`,
    );
    window.open(
      `mailto:${inquiry.senderEmail}?subject=${subject}&body=${body}`,
      "_blank",
    );
  };
...
function InquiryRow({ inquiry, expanded, onToggle, onDelete }) {
  const date = new Date(inquiry.submittedAt).toLocaleDateString("en-GB", {
    day: "numeric",
    month: "short",
    year: "numeric",
  });
  const time = new Date(inquiry.submittedAt).toLocaleTimeString("en-GB", {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  return (
    <div
      ...
      onMouseEnter={(e) =>
        (e.currentTarget.style.borderColor = "rgba(5,5,5,0.2)")
      }
      onMouseLeave={(e) =>
        (e.currentTarget.style.borderColor = "rgba(5,5,5,0.09)")
      }
    >
      {/* Row header */}
      <div
      ...
      >
        ...
        >
          <MessageSquare
            size={15}
            style={{ color: open ? "#d97706" : "#16a34a" }}
          />
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {inquiry.productTitle || "General enquiry"} —{" "}
        ...
        >
          ...
          {expanded ? (
            <ChevronUp size={15} style={{ color: "#888" }} />
          ) : (
            <ChevronDown size={15} style={{ color: "#888" }} />
          )}
        </div>
      </div>
          ...
          >
            {[
              { icon: Mail, label: "Email", value: inquiry.senderEmail },
              {
                icon: Phone,
                label: "Phone",
                value: inquiry.senderPhone || "—",
              },
              { icon: Clock, label: "Product", value: inquiry.productTitle },
              {
                icon: inquiry.status === "OPEN" ? Clock : CheckCircle,
                label: "Status",
...
export default function InquiriesPage() {
  ...
  const handleDelete = async (inq) => {
    ...
    if (
      !inq &&
      !window.confirm(
        `Delete inquiry from "${target.senderName}"? This cannot be undone.`,
      )
    )
      return;
    setDeleting(true);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        {[
          ...
          return (
            ...
            >
              <Icon
                size={12}
                style={{ color: filter === s.key ? "#FCFCFA" : s.color }}
              />
              <span
                style={{
                  fontFamily: F.ui,
                  fontSize: "1.2rem",
                  fontWeight: 800,
                  color: filter === s.key ? "#FCFCFA" : "#050505",
                  lineHeight: 1,
              ...
              <span
                style={{
                  fontFamily: F.data,
                  fontSize: "0.6875rem",
                  color: filter === s.key ? "rgba(252,252,250,0.7)" : "#555",
                  letterSpacing: "0.12em",
                  textTransform: "uppercase",
                  fontWeight: 500,
                }}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "4rem 0" }}>
          <MessageSquare
            size={32}
            style={{ color: "#ddd", margin: "0 auto 0.75rem" }}
          />
          <p
            style={{
              fontFamily: F.ui,
